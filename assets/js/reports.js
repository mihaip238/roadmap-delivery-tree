(function () {
  ...
  const budgeted = []
    .concat(
      Hours.productRows().filter((r) => r.budget != null).map((r) => ({ label: r.name, spent: r.spent, budget: r.budget })),
      Hours.milestoneRows().filter((r) => r.budget != null).map((r) => ({ label: r.key, spent: r.spent, budget: r.budget })),
      Hours.edpRows().filter((r) => r.budget != null).map((r) => ({ label: r.key, spent: r.spent, budget: r.budget }))
    )
    .sort((a, b) => (Number(b.spent) / (Number(b.budget) || 1)) - (Number(a.spent) / (Number(a.budget) || 1)));
